...
export class TicketComponent {
  ...
  constructor(private token: TokenStorageService, private ticketService: ItemsService, private fb: FormBuilder, private customerService: CustomerService, private toster: ToastrService, private couponService: BookingService) {
  ...
  private saveCustomer(customer: saveCustomer, booking: BookingForm, paymentRef: string) {
    this.loading = true
    this.ticketSubscription.add(
      this.customerService.insertCustomer(customer).subscribe({
        next: (response) => {
          console.log('Response:', response)
          this.findCustomer(booking.phone, booking, paymentRef)
        },
  ...
  private findCustomer(number: string, booking?: BookingForm, paymentRef?: string, isgenerateCoupon: boolean = false) {
